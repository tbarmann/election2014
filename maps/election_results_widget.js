...
function getSenateRaceNumber (district,show_uncontested) {
	...
	for (var x=0; x<count; x++) {
		if (re.test(ga_senator.contests[x].name)) {
			if (show_uncontested === true || ga_senator.contests[x].candidates.length > 1) {
...
function getHouseRaceNumber(district,show_uncontested) {
	...
	for (var x=0; x<count; x++) {
		if (re.test(ga_representative.contests[x].name)) {
			if (show_uncontested === true || ga_representative.contests[x].candidates.length > 1) {
...
function get_race_numbers(election_data,show_uncontested) {
	...
	for (var x=0;x<election_data.contests.length;x++) {
		if (show_uncontested === true || election_data.contests[x].candidates.length > 1) {
...
function showResultsTable(results) {
	...
	for (var row=0;row<results.candidates.length;row++) {
		...
		if ((isFinal===true) && (row<parseInt(results.votes_allowed)))
			html+='<img src="http://res.providencejournal.com/politics/primary/2012/assets/pages/gcheck.gif"/>';
		html+='</td>';
		html+='</tr>';
	}
...
var pct_precincts = (results.precincts_reporting > 0) ? (results.precincts_reporting/results.precinct_count) * 100 : 0;
...
var html = '<div id="town-header-container">\
   ...
   <div class="candidates-header-container" style="padding-top: 10px;">\
	...
	for (var row=0;row<results.candidates.length;row++) {
		...
		var pct_votes = (results.total_votes > 0) ? (results.candidates[row].votes / results.total_votes) * 100 : 0;
...
cssNode.href = 'http://res.providencejournal.com/politics/primary/2012/assets/scripts/election_results_widget.css';
