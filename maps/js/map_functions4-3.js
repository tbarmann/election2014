...
function initFields () {
	...
	if (typeof(map_init.fields) === 'undefined' || map_init.fields.length < 1) {
...
//			map_init.fields[index].mapable = $.isNumeric(map_init.datasource[0][map_init.fields[index].name]) ;
			map_init.fields[index].mapable = true;
		}
...
function colorMap(field_index) {
	...
	$("area").each (function () {
			...
			if (this_data_obj !== undefined) {
				...
				var new_class = " {strokeColor:'" + stroke_color + "',strokeWidth:" + stroke_width + ",fillOpacity:" + fill_opacity + ",alwaysOn:true,fade:false}";
				$(this).attr('class', new_class);
			}
